...
class CSP:
    def __init__(self):
        ...
        # Counters for amount of backtracks and failed backtracks
    ...
    def backtrack(self, assignment):
        ...
        pseudocode, you need to make a deerp copy of 'assignment' into a
        new variable before changing it. Every iteration of the for-loop
        should have a clean slate and not see any traces of the old
        assignments and inferences that took place in previous
        iterations of the loop.
        """
        # Check if assignment is complete (every variable in dict 'assignment' has len() = 1)
        complete = True
        ...
        # Count amount of backtracks
        self.backtracks += 1

        var = self.select_unassigned_variable(assignment)
        for val in assignment[var]:
            # Make a deep copy of assignment and assign { var = val } as noted in pseudo-code
            tmp = copy.deepcopy(assignment)
            tmp[var] = val
            # Use algorithm AC-3 from textbook
            inferences = self.inference(tmp, self.get_all_arcs())
            # Inference -> backtrack recursively
            if inferences:
                result = self.backtrack(tmp)
                # Return result if backtrack is successful
                if result:
                    return result
        # Count amount of failures in backtracking-algorithm
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        # Selects first element with len > 1 (basically randomly)
        for key, var in assignment.items():
            if len(var) != 1:
                return key
        ...
        while not len(queue) <= 0:
            ...
            if self.revise(assignment, Xi, Xj):
                ...
                if len(assignment[Xi]) == 0:
                    return False
                for Xk, _ignore_this_ in self.get_all_neighboring_arcs(Xi):
                    if not Xk == Xj:
                        queue.append([Xk, Xi])
        return True
    ...
    def revise(self, assignment, i, j):
        ...
        for x in assignment[i]:
            ...
            if len(list(filter(lambda arc: arc in arcs, self.constraints[i][j]))) < 1:
                # Delete x from 'assignment[i]' if there are no constraints satisfied by 'assignment[j]'
                revised = True
...
def print_board(board, difficulty):
    ...
    print("Solving board :", difficulty)
    print("\n")
    print_sudoku_solution(csp.backtracking_search())
    print("\n")
    print("Total amount of backtracks:", csp.backtracks, end="\n\n")
    print("Total amount of failed backtracks:", csp.failed_backtracks, end="\n\n\n")
...
if __name__ == '__main__':
    ...
    choose_board = input("Which board would you like to run? ('easy' / 'medium' / 'hard' / 'very hard' / 'all') ")

    if choose_board == "all":
        for difficulty, board in boards.items():
            print_board(board, difficulty)
    else:
        if choose_board in boards.keys():
            print_board(boards[choose_board], choose_board)
